...
A noninteractive command surface for deleting one session's PackWalk-owned
current view and evidence history, plus a separate command for clearing all
PackWalk session data.
...
- [ ] The commands are explicit, typed, and distinguish
      missing/already-deleted data from successful change.
